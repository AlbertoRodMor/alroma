...
/*AQUÍ VEMOS EL STOCK QUE HAY PARA LA BICI ELEGIDA, POR LA CLASE STOCK1,2,3....*/
for($i=1; $i<=5; $i++){
...
$hoy = date('Y-m-d');
/*SI EN EL FORMULARIO CONFIRMAMOS LA RESERVA*/
...
/*REALIZAMOS UNA CONSULTA FILTRADA POR EL NOMBRE DE LA BICI RESERVADA*/

 $pdo1 = conexion();
 $sql_reserva = "SELECT cod,stock from bicis_alquiler where nombre='".$nombre_bici_reservada."'";
...
$differenceFormat = '%a';/*AQUÍ CALCULAMOS LOS DÍAS DE DIFERENCIA ENTRE LAS DOS FECHAS LA DE INICIO Y LA DE FIN, PARA CALCULAR EL IMPORTE*/

 $diasAlquilar = $contador->format($differenceFormat);
 $totalImporte = ($diasAlquilar * 10);/*CADA DÍA ES 10 EUROS POR BICI*/
 
 $total = $cantidadDeseada * $totalImporte;

 $stockTotal = $row3['stock'];
 $stockRestante = $stockTotal-$cantidadDeseada;
  $pdo1 = conexion();/*INSERTAMOS LA RESERVA CON LOS DATOS DEL ARRENDATARIO*/
		...
		<div class="row">
			...
			<div class="col-12 d-flex justify-content-center mt-5">
				<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" id="formulario_datos_arrendatario">
					...
					<input type="date" name="fechaInicio" value="<?php if(isset($_POST['fechaClickada'])){ echo $_POST['fechaClickada']; } ?>" min="<?php if(isset($_POST['fechaClickada'])){ echo $_POST['fechaClickada']; } ?>" required >
					<input type="date" name="fechaFin" required min="<?php if(isset($_POST['fechaClickada'])){ echo $_POST['fechaClickada']; } ?>"><br>
