...
jQuery(document).ready(function(){
	...
	jQuery('#formulario_insertar_trabajador').on("submit",function(){
		var url ="php/validation-home-trabajadores.php";

		var nombreTrabajador=jQuery('.insertNombreTr').val();  
		var ApellTr=jQuery('.insertApellTr').val();
		var UsuarioTr=jQuery('.insertUsuarioTr').val();
		var PassTr=jQuery('.insertPassTr').val();

		var longUsuario = jQuery('.insertUsuarioTr').val().length;
		var longPass = jQuery('.insertPassTr').val().length;
			jQuery.ajax({

				type : 'POST',
				url : url,
				data : {'nombreTrabajador': nombreTrabajador,'ApellTr': ApellTr,'UsuarioTr':UsuarioTr,'PassTr':PassTr},

				success: function(data){
					console.log(longUsuario+" " + longPass);
						if(data){
	...
	/*INSERTAR FACTURA*/
	jQuery('.crear_facturas_noregistro').click(function(){
		if(jQuery('.contenedor-crear-facturas-noregistro').hasClass('d-none')){

	 		jQuery('.contenedor-crear-facturas-noregistro').removeClass('d-none');
	 		jQuery('.contenedor-crear-facturas-noregistro').addClass('d-block');
	 	}else{
	 		jQuery('.contenedor-crear-facturas-noregistro').removeClass('d-block');
	 		jQuery('.contenedor-crear-facturas-noregistro').addClass('d-none');
